  ...
  if (sAlgorithm == "pfCombinedInclusiveSecondaryVertexV2BJetTags") 
    {
      ...
      if (sWorkingPoint == "Loose")       fDisriminatorValue = 0.605;
      else if (sWorkingPoint == "Medium") fDisriminatorValue = 0.890;
      else if (sWorkingPoint == "Tight")  fDisriminatorValue = 0.970;
    } 
  else if (sAlgorithm == "pfCombinedSecondaryVertexBJetTags") 
    {
      // Run 1 legacy values
      if (sWorkingPoint == "Loose")        fDisriminatorValue = 0.244;
      else if (sWorkingPoint == "Medium")  fDisriminatorValue = 0.679;
      else if (sWorkingPoint == "Tight")   fDisriminatorValue = 0.898;
    }
  else if (sAlgorithm == "pfJetProbabilityBJetTags") 
    {
      // Oct 2015 https://indico.cern.ch/event/455330/session/1/contribution/173/attachments/1175466/1699105/DMajumder_CMSWeek22Oct_BTVReport_v1.pdf
      if (sWorkingPoint == "Loose")       fDisriminatorValue = 0.275;
      else if (sWorkingPoint == "Medium") fDisriminatorValue = 0.545;
      else if (sWorkingPoint == "Tight")  fDisriminatorValue = 0.790;    
    }
...
void BJetSelection::bookHistograms(TDirectory* dir) {
  ...
  hSelectedBJetPt.push_back(  fHistoWrapper.makeTH<TH1F>( HistoLevel::kDebug, subdir, "selectedBJetsFirstJetPt"  , "First b-jet pT"   , 40,  0.0, 400.0) );
  hSelectedBJetPt.push_back(  fHistoWrapper.makeTH<TH1F>( HistoLevel::kDebug, subdir, "selectedBJetsSecondJetPt" , "Second b-jet pT"  , 40,  0.0, 400.0) );
...
BJetSelection::Data BJetSelection::analyze(const Event& event, const JetSelection::Data& jetData) {
  ...
  if (fCommonPlots != nullptr) fCommonPlots->fillControlPlotsAtBtagging(event, data);
...
BJetSelection::Data BJetSelection::privateAnalyze(const Event& iEvent, const JetSelection::Data& jetData) {
  ...
  // For-loop: All "Selected Jets". Jet pt & eta cuts expected to be the same like for the selected jets for simplicity
  for(Jet jet: jetData.getSelectedJets()) {

    //====== Apply Cut: discriminator
    if ( !(jet.bjetDiscriminator() > fDisriminatorValue) ) continue;
    passedDisr = true;
  ...
  //====== Apply Cut: number of selected b jets
  if ( !cfg_NJetsCut.passedCut(output.getNumberOfSelectedBJets()) ) return output;
  ...
  // FIXME (To be implemented): Calculate and store b-jet scale factor weight and it's uncertainty
