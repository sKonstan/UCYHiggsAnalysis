...
#include "MiniAOD2FlatTree/interface/NtupleAnalysis_fwd.h"
...
public:
  ...
  bool isElectronToTau() const { 
    return this->pdgId() == TauDecayMatchType::kElectronToTau || this->pdgId() == TauDecayMatchType::kTauDecaysToElectron; 
  }

  bool isMuonToTau() const { 
    return this->pdgId() == TauDecayMatchType::kMuonToTau || this->pdgId() == TauDecayMatchType::kTauDecaysToMuon; 
  }

  bool isJetToTau() const { 
    return (this->pdgId() >= TauDecayMatchType::kUToTau && this->pdgId() <= TauDecayMatchType::kBToTau) || this->pdgId() == TauDecayMatchType::kGluonToTau; 
  }
  ...
  bool configurableDiscriminators() const {
    for(auto& disc: fCollection->fConfigurableDiscriminators) {
      if( !disc->value()[index()] ) return false;
    }
    return true;
  }

  bool againstElectronDiscriminator() const {
    if ( !fCollection->againstElectronDiscriminatorIsValid() ) return true;
    return fCollection->fAgainstElectronDiscriminator->value()[index()];
  }

  bool againstMuonDiscriminator() const {
    if ( !fCollection->againstMuonDiscriminatorIsValid() ) return true;
    return fCollection->fAgainstMuonDiscriminator->value()[index()];
  }

  bool isolationDiscriminator() const {
    if ( !fCollection->isolationDiscriminatorIsValid() ) return true;
    return fCollection->fIsolationDiscriminator->value()[index()];
  }

  /// Operator defined for using std::sort on vector<Tau>. Descending order by tau pT
  bool operator<(const Tau& tau) const { return ( this->pt() > tau.pt() ); }
