...
//****************************************************************************
{
  ...
  for(int i = 0; i < nHits; i++){
    ...
    TVector3 candPixHit_XYZ( candPixHits_X.at(i), candPixHits_Y.at(i), candPixHits_Z.at(i) );
...
void L1PixelTrackFit::Multifit(double rinv,
			       ...
			       double t,
			       double z0,
			       std::vector<TVector3> hitL1,
			       std::vector<TVector3> hitL2,
			       std::vector<TVector3> hitL3,
			       std::vector<TVector3> hitL4,
			       std::vector<TVector3> hitD1,
			       std::vector<TVector3> hitD2,
			       std::vector<TVector3> hitD3,		
			       bool& success,
			       double& invrfinal,
			       double& phi0final,
			       double& d0final,
...
//****************************************************************************
{
  ...
  do {
    more=false;
	 ...
	 more=true;
	 std::vector<TVector3> tmp=hits[i];
	 hits[i]=hits[i+1];
	 hits[i+1]=tmp;
	 bool tmpb=barrel[i];
	 barrel[i]=barrel[i+1];
  ...
  if(bPrintBeforeSort){
	...
	std::cout << "\t"<< j << ") r = " << hits[i].at(j).Perp() << ", z = " << hits[i].at(j).z() << ", Phi = " << hits[i].at(j).Phi() << std::endl; 
      }
      std::cout << "" << std::endl;
    }
  }
...
//****************************************************************************
{
  ...
  if (fabs(d0) < 10.0){ 
    ...
    //  	     sigmarinv, sigmaphi0, sigmad0, sigmat, sigmaz0, pixHits, 4);
    //
    // if(!success){ // If no 4 hits fit possible re-do enabling 3 hits fits
    //   Multifit(rinv, phi0, d0, t, z0,
    //   	       pixHitL1, pixHitL2, pixHitL3, pixHitL4, pixHitD1, pixHitD2, pixHitD3,
    //   	       success, invrfit, phi0fit, d0fit, tfit, z0fit, chisqfit, nhit,
    //   	       sigmarinv, sigmaphi0, sigmad0, sigmat, sigmaz0, pixHits, 3);
  ...
  TVector3 thePOCA(-d0fit * sin(phi0fit), d0fit * cos(phi0fit), z0fit );
