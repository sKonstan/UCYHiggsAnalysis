...
MiniAOD2FlatTreeFilter::MiniAOD2FlatTreeFilter(const edm::ParameterSet& iConfig) :
  ...
  eventInfoCollections(iConfig.getParameter<edm::ParameterSet>("EventInfo"))
{
  ...
  // Call the "Trigger" dumper object. 
  trgDumper = 0;
  ...
  // Call the "METNoiseFilter" dumper object.    
  ...
  // Call the "Electrons" dumper object.
  electronDumper = 0;
  if (iConfig.exists("Electrons")) {
    ...
    edm::LogInfo("MiniAOD2FlatTreeFilter") << "MiniAOD2FlatTreeFilter::MiniAOD2FlatTreeFilter() Electrons";
	
  } else {
    std::cout << "Config: ElectronDumper ignored, because 'Electrons' is missing from config" << std::endl;
  }

  // Call the "Muons" dumper object.
  muonDumper = 0;
  ...
  if(this->isMC()){
    ...
    // Call the "GenMETs" dumper object.
    ...
    // Call the "GenWeights" dumper object.
    ...
    // Call the "GenParticles" dumper object.
    ...
    // Call the "GenJets" dumper object.
...
MiniAOD2FlatTreeFilter::~MiniAOD2FlatTreeFilter() {
  system("ls -lt");
}

void MiniAOD2FlatTreeFilter::beginRun(edm::Run const & iRun, edm::EventSetup const& iSetup) {
  bool changed = true;
...
void MiniAOD2FlatTreeFilter::beginJob(){
  return;
}   

bool MiniAOD2FlatTreeFilter::filter(edm::Event& iEvent, const edm::EventSetup& iSetup){
  reset();
...
void MiniAOD2FlatTreeFilter::reset(){
...
#include <time.h>
#include "TH1F.h"
void MiniAOD2FlatTreeFilter::endJob(){
...
void MiniAOD2FlatTreeFilter::endLuminosityBlock(const edm::LuminosityBlock & iLumi, const edm::EventSetup & iSetup) {
  if(skimDumper) skimDumper->fill(iLumi,iSetup);
  
  return;
}

bool MiniAOD2FlatTreeFilter::isMC(){
  std::regex data_re("data");
  if(std::regex_search(dataVersion, data_re)) return false;
  return true;
}          
DEFINE_FWK_MODULE(MiniAOD2FlatTreeFilter);
