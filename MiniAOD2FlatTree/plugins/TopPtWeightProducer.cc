...
class TopPtWeightProducer : public edm::one::EDProducer<edm::one::SharedResources> {
public:
  explicit TopPtWeightProducer(const edm::ParameterSet&);
  ~TopPtWeightProducer();
...
TopPtWeightProducer::TopPtWeightProducer(const edm::ParameterSet& iConfig):
...
TopPtWeightProducer::~TopPtWeightProducer(){ }

void TopPtWeightProducer::beginJob() {
  hTopPtWeightAllEvents->Fill(0.5, 1);
}

void TopPtWeightProducer::endJob() {
...
void TopPtWeightProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup){
