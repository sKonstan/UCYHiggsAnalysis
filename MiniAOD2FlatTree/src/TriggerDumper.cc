...
void TriggerDumper::book(const edm::Run& iRun, HLTConfigProvider hltConfig){
  ...
  for(size_t i = 0; i < triggerBits.size(); ++i){
    // std::cout << "triggerBits["<<i<<"] = " << triggerBits[i] << std::endl;
  ...
  for(size_t imatch = 0; imatch < trgMatchStr.size(); ++imatch){
    ...
    name+= "_TrgMatch_";
    // std::cout << "trgMatchStr["<<imatch<<"] = " << trgMatchStr[imatch] << std::endl;
    
    std::regex match_re(trgMatchStr[imatch]);
    // For-loop: All selected triggers
    for(size_t i = 0; i < selectedTriggers.size(); ++i){		
      // std::cout << "selectedTriggers["<<i<<"] = " << selectedTriggers[i] << std::endl;
	...
	for(size_t j = 0; j < trgMatchBranches.size(); ++j){
	  // std::cout << "trgMatchBranches["<<j<<"] = " << trgMatchBranches[j] << std::endl;	  
  ...
  if(trgResultsHandle.isValid()){
    ...
    if (cfg_debugMode){
      ...
      std::cout << std::setw(5)       << "Index"  << std::setw(width*7) << "Collection" << std::setw(width)   << "L1ETM "
		<< std::setw(width)   << "MET "   << std::setw(width)   << "Tau "       << std::setw(width)   << "Fired?" 
	...
	if(patTriggerObject.id(trigger::TriggerTau)){
	  ...
	  if (cfg_debugMode) std::cout << index << std::setw(width*7) << patTriggerObject.collection() 
				       << std::setw(width) << bL1ETM  << std::setw(width) << bMET << std::setw(width) << bTau << std::setw(width) << fired 
